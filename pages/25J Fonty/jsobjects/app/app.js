export default {
	...
	loadingData: false,
	sections: ["section_e1", "section_e2", "section_e3", "section_e4"],
	list_courses: {
		section_e1: list_courses_e1,
		section_e2: list_courses_e2,
		section_e3: list_courses_e3,
		section_e4: list_courses_e4,
	},
	list_assignments: {
		section_e1: list_assignments_e1,
		section_e2: list_assignments_e2,
		section_e3: list_assignments_e3,
		section_e4: list_assignments_e4,
	},
	excludedCourseIds: [
		8054, // Business Foundations
	],
	rawAssignments: {},
	courses: {},
	assignments: [],
	...
	unifyAllAssignments () {
		...
		this.sections.forEach((section) => {
			this.rawAssignments[section].forEach((rawAssignment) => {
				const uniqueDueId = `${rawAssignment.id}~~${rawAssignment.due_at}`;
				const index = assignments.findIndex(a => a.uniqueDueId === uniqueDueId);
				if (index > -1) {
					assignments[index].sections.push(section);
				} else {
					assignments.push({ ...rawAssignment, uniqueDueId, sections: [section] });
				}
			});
		});
	...
	cleanAssignments () {
		this.assignments = this.assignments.map((assignment) => {
			const course = this.courses[assignment.sections[0] ?? this.sections[0]].find(course => course.id === assignment.course_id);
			const allSectionsPresent = this.sections.reduce((agg, s) => agg && assignment.sections.includes(s), true);
			const sectionsPrefix = `[${assignment.sections.map(s => s.replace("section_", "sec-").toUpperCase()).join(" / ")}]`;
			return {
				course: course.name,
				name: allSectionsPresent ? assignment.name : `${sectionsPrefix} ${assignment.name}` ,
				htmlUrl: assignment.html_url,
				// description: assignment.description,
				dueAt: assignment.due_at,
				groupType: assignment.group_category_id || assignment.name.toLowerCase().includes("(group)") ? "Group" : "Individual",
